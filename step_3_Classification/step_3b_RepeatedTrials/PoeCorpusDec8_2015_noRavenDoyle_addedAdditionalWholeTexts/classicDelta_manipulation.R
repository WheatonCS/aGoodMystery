	...
	for (i in authors)
	{
		...
		if(length(temp) > 1) {
			...
			for (k in 2: maxToTest){
				if (temp[k] != "Poe_ADream_1831.txt" & !(alreadyTestingTwoForPoe == TRUE & k==2)){
		...
		} # if only one text by this author in the corpus
		
		# else an author only has one text, into the PRIMARY (train) set you go
		else{
			filename= paste("./",temp[1], sep="")
			file.copy(filename, "../primary_set/")
